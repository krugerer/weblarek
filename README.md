...
`submit-form`: `Form<T>` - Возникает при отправке любой формы после успешной валидации
`order`: `Basket` - Возникает при клике на кнопку «Оформить» в корзине
`basket-open`: `Header` - Возникает при клике на кнопку корзины в шапке сайта
Клик по карточке: `Card<T>` - Передаётся через `ICardActions.onClick`
Добавление в корзину: `Card<T>` - Передаётся через `ICardActions.onAddToBasket`

### Презентер

Отвечает за связь моделей данных и представлений. Обрабатывает события, которые генерируются в моделях и представлениях, и выполняет соответствующие действия.

Расположение: Код презентера находится в `main.ts` (отдельный класс не создаётся, так как приложение имеет только одну страницу).
...
2. Представления перерисовываются только при изменении данных или открытии модального окна
3. Все действия пользователя -> события → презентер -> вызов методов моделей -> новые события -> обновление представлений
4. Данные хранятся только в моделях, презентер их только читает и передаёт между слоями
...
`catalog:changed`: `Catalog` - Отрисовка галереи товаров на главной странице
`basket:changed`: `Basket` - Обновление счётчика в шапке; если корзина открыта — перерисовка её содержимого
`customer:changed`: `Customer` -  (Резерв, используется при отправке заказа)

### События от представлений

`card:select`: `Card` (каталог) - Найти товар в модели каталога, открыть модальное окно с карточкой `CardPreview`
`card:add-to-basket`: `Card` (каталог, превью) - Найти товар в модели каталога, добавить в корзину
`basket:open`: `Header` - Открыть модальное окно с содержимым корзины
`basket:remove`: `CardBasket` - Удалить товар из корзины; если корзина открыта — обновить её отображение
`order:submit`: `BasketView` - Открыть форму заказа (`OrderForm`) с подстановкой сохранённых данных
`order:change`: `OrderForm` - Сохранить данные заказа (способ оплаты и адрес) в модель `CustomerModel`
`order:submit-form`: `OrderForm` - После валидации формы заказа — открыть форму контактов (`ContactsForm`)
`contacts:change`: `ContactsForm` - Сохранить контакты (email, телефон) в модель `CustomerModel`
`contacts:submit`: `ContactsForm` - Собрать данные заказа (из корзины и модели покупателя), отправить на сервер через `AppApi.postOrder`, показать окно успеха `Success`, очистить корзину и данные покупателя
`success:close`: `Success` - Закрыть модальное окно
`modal:close`: `Modal` - Сбросить флаг открытой корзины

### Вспомогательные функции

`renderBasket()` - Отрисовывает содержимое корзины: создаёт карточки `CardBasket` для каждого товара, обновляет общую сумму, открывает модальное окно
`createCardCatalog(product: IProduct): HTMLElement` - Создаёт карточку каталога `CardCatalog`, заполняет данными товара, возвращает DOM-элемент для вставки в галерею

## Поток данных в приложении

1. Загрузка товаров:  
   `AppApi.getProducts()` -> `Catalog.setItems()` -> событие `catalog:changed` -> презентер отрисовывает галерею

2. Просмотр товара:  
   Клик по карточке -> событие `card:select` -> презентер открывает модалку с `CardPreview`

3. Добавление в корзину:  
   Клик на кнопку «В корзину» -> событие `card:add-to-basket` -> `Basket.addProduct()` -> событие `basket:changed` -> презентер обновляет счётчик в шапке

4. Оформление заказа:  
   Клик на корзину -> `basket:open` -> презентер вызывает `renderBasket()` -> открывается корзина  
   Клик «Оформить» -> `order:submit` -> открывается форма заказа  
   Заполнение адреса и выбор оплаты -> `order:change` -> сохранение в `CustomerModel`  
   Клик «Далее» -> `order:submit-form` -> открывается форма контактов  
   Заполнение email и телефона -> `contacts:change` -> сохранение в `CustomerModel`  
   Клик «Оплатить» -> `contacts:submit` -> отправка заказа на сервер -> показ окна успеха → очистка корзины и данных покупателя


## Особенности реализации

- Корзина перерисовывается сразу при добавлении, удалении или очистке, если она открыта (флаг `isBasketOpen`)
- Изображения товаров загружаются с использованием `CDN_URL` из `constants.ts`
- Валидация форм происходит на стороне клиента перед отправкой
- После успешного заказа корзина и данные покупателя очищаются, счётчик в шапке сбрасывается